...
import { alertTypeForRule, decide, type Dispositions, type ExistingIncident, type FindingRow }
  from '../src/alerts/finding-pipeline.js'

const READ_LIMIT = 5000            // mirrors findOpenFindings in alert-intake.service.ts
...
const findings: FindingRow[] = rows.map((row) => ({
  ...
  ruleId: row.rule_id, subjectType: row.subject_type, subjectId: row.subject_id,
...
if (organizationIds.length > 0) {
  for (const r of await prisma.$queryRawUnsafe<{ organization_id: string; rule_id: string; disposition: string }[]>(
    'SELECT organization_id, rule_id, disposition FROM alert_rule_dispositions WHERE organization_id = ANY($1::uuid[])',
    organizationIds)) byOrganizationAndRule.set(`${r.organization_id}|${r.rule_id}`, r.disposition)
...
const dispositions: Dispositions = { byOrganizationAndRule, anyRecipientByOrganization }
