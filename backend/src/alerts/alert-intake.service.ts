...
export class AlertIntakeService {
  ...
  async runOnce(deadlineAt: number, tickAt: Date = new Date()): Promise<IntakeOutcome> {
    ...
    this.logger.log(JSON.stringify({
      ...
      // **A SILENCE SOMEBODY CHOSE, COUNTED WHERE THE OTHER OUTCOMES ARE.** Without it, a tick
      // that told nobody about anything and a tick with nothing to say produce the same line, and
      // a disposition set on the wrong type is invisible until an MSP asks why it went quiet.
      noticesWithheld: report.noticesWithheld,
