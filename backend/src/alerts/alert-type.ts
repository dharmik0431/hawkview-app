 ...
 * re-think. See `project documentation`.
 *
 * ⚠ **ITS `RECORD_ONLY` ANSWER IS STALE, AND THE FUNCTION HAS NO CALLER BUT ITS OWN TEST.**
 * `IN_APP` was right while a record still rang the bell. The ruling — *retain evidence and
 * inspectable history, but suppress active notifications, unread bell counts, and email* — makes
 * a record route NOWHERE, and `decide` implements that by writing no notification row at all.
 *
 * NOT CORRECTED HERE, because the delivery path this feeds is frozen and unbuilt, and changing a
 * value nothing reads would be a claim rather than a fix. Whoever builds that path must not read
 * `IN_APP` as the ruled answer.
 *
 * THIS IS NOT THE ZERO-CALLER DEFECT THIS FEATURE KEEPS FINDING, and the difference is worth
 * keeping. Those were functions believed to be wired and were not. This one is unreachable ON
 * PURPOSE — the paragraph above says so — which is exactly why nothing failed when the ruling
 * moved out from under it. **An intentionally unreachable function still holds claims, and
 * nothing will fail when they go stale.** */
