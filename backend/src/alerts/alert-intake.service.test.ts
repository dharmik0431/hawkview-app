...
test('A VALID WATERMARK GETS PAST THE REFUSAL, or the two tests above prove nothing', async () => {
  ...
  // Observe the read boundary itself. Returning the same sentinel from both
  // success and catch cannot distinguish a valid gate from an always-off gate.
  assert.equal(reads, 1, 'a valid watermark reaches the synthetic finding reader')
  assert.notEqual(outcome.kind, 'NOT_CONFIGURED')
})
