...
import {
  ...
  asAlertTypeId, dispositionKey, noticeKeyFor, scopedNoticeKey,
...
test('A COMMIT THAT FAILS REPORTS THE PHASE AND EVERYTHING IT LOST', async () => {
  ...
  const store: PipelineStore = {
    ...
    findDecidedNoticeKeys: async () => new Set<string>(),
...
test('AND A HEALTHY TICK IS NEITHER, or the two above are satisfied by always failing', async () => {
  const store: PipelineStore = {
    ...
    countUnknownAlertTypes: async () => 0,
    commit: async () => ({
      incidentsWritten: 1, notificationsWritten: 1, jobsWritten: 1, noticesWithheld: 0,
    }),
  }
  const outcome = await runIntake(store, WATERMARK, T0, Date.now() + 30_000, '2026-01-01T00:00:00.000Z')
...
test('LOWERING A TYPE TO THE QUIETEST TIER SILENCES IT ENTIRELY', () => {
  // **THIS TEST USED TO ASSERT THE OPPOSITE, AND IT WAS RIGHT AT THE TIME.** It read: *the
  // incident and the notification still exist — off means recorded, not absent*, and asserted a
  // notification with severity `info`. Under the ruled definition of record-only that is now
  // false: off means recorded, NOT visible, NOT counted, NOT emailed. Rewritten rather than
  // deleted, so the change of meaning is visible to whoever reads the history.
  const mapped = ALERT_CATALOG.find((type) => type.id === 'security.suspected_credential_attack')
  assert.ok(mapped !== undefined)
  assert.equal(mapped.severity, 'ACT_NOW', 'the catalogue disagrees, so this is the MSP overriding')

  const silenced: Dispositions = {
    ...canEmail,
    byOrganizationAndAlertType: new Map([[dispositionKey(ORG, mapped.id), 'RECORD_ONLY']]),
  }
  const out = decide([finding()], none, silenced, WATERMARK, T0)

  assert.equal(out.incidents.length, 1, 'the evidence and the inspectable history remain')
  assert.equal(out.notifications.length, 0, 'no inbox entry, and so no unread count')
  assert.equal(out.jobs.length, 0, 'and no email')
  assert.equal(out.skipped[0]?.because, 'RECORD_ONLY')

  // AND THE OTHER TWO TIERS ARE STILL DISTINGUISHABLE FROM EACH OTHER, or "the setting does
  // something" would be satisfied by RECORD_ONLY alone and the middle tier would be inert again.
  const today: Dispositions = {
    ...canEmail,
    byOrganizationAndAlertType: new Map([[dispositionKey(ORG, mapped.id), 'ACT_TODAY']]),
  }
  const lowered = decide([finding()], none, today, WATERMARK, T0)
  assert.equal(lowered.notifications[0]?.severity, 'high', 'not critical, which is the catalogue')
  assert.equal(lowered.jobs.length, 1, 'ACT_TODAY still delivers')
})
...
test('THE TICK COMMITS IN CHUNKS, one transaction each', async () => {
  ...
  const store: PipelineStore = {
    ...
    findDecidedNoticeKeys: async () => new Set<string>(),
    countUnknownAlertTypes: async () => 0,
    commit: async (incidents, notifications, jobs, withheld) => {
      ...
      return {
        ...
        noticesWithheld: withheld.length,
      }
    },
  }
...
test('RECORD_ONLY KEEPS THE EVIDENCE AND SUPPRESSES THE NOTICE', () => {
  // **THE DEFINITION, VERBATIM:** *retain evidence and inspectable history, but suppress active
  // notifications, unread bell counts, and email.* The incident row IS the evidence; the
  // notification row is the active notice. It used to write both and withhold only the job, so
  // "off" meant recorded, visible in the bell and the inbox, not emailed.
  //
  // NO ROW MEANS NO INBOX ENTRY AND NO UNREAD COUNT, so the bell requirement is satisfied by
  // construction rather than by a second filter at read time — a filter would be a second place
  // holding the fact, which this feature has now fixed four times.
  const off: Dispositions = {
    ...canEmail,
    byOrganizationAndAlertType: new Map([
      [dispositionKey(ORG, 'security.suspected_credential_attack'), 'RECORD_ONLY'],
    ]),
  }
  const out = decide([finding()], none, off, WATERMARK, T0)

  assert.equal(out.incidents.length, 1, 'THE EVIDENCE REMAINS — inspectable history')
  assert.equal(out.notifications.length, 0, 'AND THE NOTICE IS SUPPRESSED')
  assert.equal(out.jobs.length, 0, 'email was already suppressed and stays so')
  assert.equal(out.skipped[0]?.because, 'RECORD_ONLY')

  // THE CONTROL: with the type on, all three appear. Without it the assertions above are
  // satisfied by a pipeline that writes nothing.
  const on = decide([finding()], none, canEmail, WATERMARK, T0)
  assert.equal(on.incidents.length, 1)
  assert.equal(on.notifications.length, 1)
  assert.equal(on.jobs.length, 1)
})

/** What the store would return after a tick that withheld these findings. The two tables the
 * real `findDecidedNoticeKeys` unions are both keyed this way, so a fake that builds the set from
 * the findings is the same shape rather than a convenient stand-in. */
const decidedAbout = (...findings: readonly FindingRow[]): ReadonlySet<string> =>
  new Set(findings.map((each) => scopedNoticeKey(each.organizationId, noticeKeyFor(each))))

test('RE-ENABLING DOES NOT REPLAY WHAT WAS SUPPRESSED', () => {
  // **THE HARDER HALF, AND THE DEDUPE DOES NOT PROVIDE IT.** The notification is pushed before the
  // already-open check, so a finding suppressed while the type was off would get its notification
  // the moment the type came back — an MSP re-enabling and being buried in a month of backlog,
  // which is the watermark's failure mode and just as unrecallable.
  //
  // The job half IS provided by the dedupe, because the job push sits after the `continue`. Half a
  // guarantee is what makes this worth stating rather than assuming.
  const suppressed = finding({ id: 'f-1', subjectId: 'user-1', dedupeKey: 'd-1' })
  const off: Dispositions = {
    ...canEmail,
    byOrganizationAndAlertType: new Map([
      [dispositionKey(ORG, 'security.suspected_credential_attack'), 'RECORD_ONLY'],
  ...
  const whileOff = decide([suppressed], none, off, WATERMARK, T0)
  assert.equal(whileOff.incidents.length, 1, 'THE EVIDENCE REMAINS')
  assert.equal(whileOff.notifications.length, 0)

  // **THE DECISION IS WRITTEN DOWN, and that is what makes the next part true rather than lucky.**
  // Without this row nothing distinguishes a finding suppressed while the type was off from one
  // that arrived after it came back on — both are OPEN, both are in the window, both have no
  // notification, and the incident is open in both worlds.
  assert.deepEqual(whileOff.withheld, [{
    organizationId: ORG,
    dedupeKey: 'identity-risk:d-1',
    alertTypeId: 'security.suspected_credential_attack',
    findingId: 'f-1',
    because: 'RECORD_ONLY',
  }])

  // The MSP turns it back on. The finding is still OPEN, so the next tick reads it again — and
  // finds its own earlier answer.
  const afterReEnabling = decide(
    [suppressed], none, canEmail, WATERMARK, T0, decidedAbout(suppressed))

  assert.equal(afterReEnabling.notifications.length, 0, 'NO REPLAY — the backlog stays silent')
  assert.equal(afterReEnabling.jobs.length, 1, 'the email is a separate question, and unchanged')
  assert.equal(afterReEnabling.withheld.length, 0, 'and the decision is not retaken')

  // THE CONTROL. Without the recorded decision the very same call DOES notify, so the assertion
  // above is about the record and not about something else in the tick.
  const withoutTheRecord = decide([suppressed], none, canEmail, WATERMARK, T0)
  assert.equal(withoutTheRecord.notifications.length, 1)
})

// ⚠ THIS TEST DOES NOT ESTABLISH THE SAME-INCIDENT CASE, and it looked as though it did. The new
// finding below is on a DIFFERENT SUBJECT, so it is a different incident and never meets the gate
// at all. The case that matters — new evidence on an incident that is already open — is the
// failing `R001-A` test above. A test can be green and about the wrong thing.
test('OFF THEN ON MEANS START TELLING ME, not stay silent forever', () => {
  // The other direction, and it is the one that would make the setting useless: a type re-enabled
  // must alert on what happens NEXT. Without this, "no replay" could be satisfied by never
  // notifying again.
  const old = finding({ id: 'f-1', subjectId: 'user-1', dedupeKey: 'd-1' })
  const off: Dispositions = {
    ...canEmail,
    byOrganizationAndAlertType: new Map([
      [dispositionKey(ORG, 'security.suspected_credential_attack'), 'RECORD_ONLY'],
  ...
  decide([old], none, off, WATERMARK, T0)

  // A NEW finding ON THE SAME SUBJECT — so the SAME INCIDENT, which is the case the earlier
  // version of this test missed by using a different subject and which R001-A is about.
  const fresh = finding({ id: 'f-2', subjectId: 'user-1', dedupeKey: 'd-2' })
  const after = decide([old, fresh], none, canEmail, WATERMARK, T0, decidedAbout(old))

  assert.equal(after.notifications.length, 1, 'the new one is told')
  assert.equal(after.notifications[0]?.dedupeKey, 'identity-risk:d-2')
  // AND THE SUPPRESSED ONE IS STILL SILENT in the very same tick, on the very same incident, so
  // the two rules coexist rather than one overriding the other.
  assert.ok(!after.notifications.some((each) => each.dedupeKey === 'identity-risk:d-1'))
})

test('A BURST INSIDE ONE TICK STILL PRODUCES A ROW EACH', () => {
  // The rule that makes no-replay work is *an incident opened by a PREVIOUS tick gets no new
  // notification*. This is the case it must not break: two findings on one incident arriving
  // together are one burst and are both worth showing.
  const first = finding({ id: 'f-1', subjectId: 'user-1', dedupeKey: 'd-1' })
  const second = finding({ id: 'f-2', subjectId: 'user-1', dedupeKey: 'd-2' })
  const out = decide([first, second], none, canEmail, WATERMARK, T0)

  assert.equal(out.incidents.length, 1, 'one incident')
  assert.equal(out.notifications.length, 2, 'AND A ROW EACH — the incident is the set of its rows')
  assert.equal(out.skipped[0]?.because, 'INCIDENT_ALREADY_OPEN')
})

test('R001-A: NEW EVIDENCE ON AN INCIDENT THAT IS ALREADY OPEN MUST STILL BE TOLD', () => {
  // Raised by a second reviewer against the no-replay gate. `openBeforeTick` is keyed by
  // organisation + INCIDENT, and an incident open from any earlier tick makes that predicate
  // permanently true — so suppressed backlog and genuinely new evidence are indistinguishable and
  // both stay silent forever. A different SUBJECT is a different incident and does not test this.
  const first = finding({ id: 'f-1', subjectId: 'user-1', dedupeKey: 'd-1' })
  const tickOne = decide([first], none, canEmail, WATERMARK, T0)
  assert.equal(tickOne.notifications.length, 1, 'control: the first one is told')

  const alreadyOpen = tickOne.incidents.map((each) => ({
    organizationId: each.organizationId, incidentKey: each.incidentKey,
  }))
  // A LATER TICK, THE TYPE NEVER SILENCED, and a genuinely new finding on the same subject —
  // material escalation on an incident somebody is already looking at.
  const escalation = finding({ id: 'f-2', subjectId: 'user-1', dedupeKey: 'd-2' })
  const tickTwo = decide([escalation], alreadyOpen, canEmail, WATERMARK, T0)

  assert.equal(tickTwo.notifications.length, 1, 'THE NEW EVIDENCE IS SURFACED')
})

test('runIntake CONSULTS THE RECORDED HISTORY, and asks about every finding it read', async () => {
  // **THE ONE PLACE THE ARGUMENT COULD BE DROPPED.** `decide` defaults `alreadyDecided` to empty,
  // which is honest for a direct call and silently disables no-replay if `runIntake` stops passing
  // it. That mutation is invisible to every `decide`-level test in this file, so it is held here:
  // delete the argument at the call site and this test fails.
  const suppressed = finding({ id: 'f-1', subjectId: 'user-1', dedupeKey: 'd-1' })
  const fresh = finding({ id: 'f-2', subjectId: 'user-1', dedupeKey: 'd-2' })
  let askedAbout: readonly string[] = []
  const store: PipelineStore = {
    findOpenFindings: async () => [suppressed, fresh],
    findExistingIncidents: async () => [],
    loadDispositions: async () => canEmail,
    findDecidedNoticeKeys: async (_orgs, keys) => {
      askedAbout = keys
      return decidedAbout(suppressed)
    },
    countUnknownAlertTypes: async () => 0,
    commit: async (incidents, notifications, jobs, withheld) => ({
      incidentsWritten: incidents.length,
  ...
  assert.equal(outcome.kind, 'RAN')
  if (outcome.kind !== 'RAN') return
  // ONE notification, for the finding with no recorded decision — not two, and not zero.
  assert.equal(outcome.report.notificationsWritten, 1)

  // AND IT ASKED ABOUT EVERY FINDING IT READ. Asking about only some would make the gate depend on
  // read order, which is the kind of thing that works until a chunk boundary moves.
  assert.deepEqual([...askedAbout].sort(), ['identity-risk:d-1', 'identity-risk:d-2'])
})
