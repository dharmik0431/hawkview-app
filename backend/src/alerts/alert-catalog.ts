...
export const ALERT_CATALOG = [
  ...
  // ── Routine directory change ───────────────────────────────────────────────
  {
    ...
    // ⚠ **UNRULED, AND IT BITES WHEN THE PUBLISH PATH IS BUILT.** `RECORD_ONLY` below is declared
    // by the PRODUCT, not chosen by an MSP — and the ruling for that tier is now *retain evidence
    // and inspectable history, but suppress active notifications, unread bell counts, and email.*
    // So the day something publishes this type, it becomes evidence-only by default: recorded,
    // and in nobody's bell, with no one having chosen that.
    //
    // NOBODY HAS RULED ON WHETHER THAT IS RIGHT. It cannot arise today — intake produces only
    // `security.suspected_credential_attack` and `security.privileged_directory_change`, both
    // declared ACT_NOW — which is exactly why it needs to be written here rather than remembered.
    // The collection-recovered type below carries the same question and answers it more easily:
    // its own declaration calls today's behaviour "good news filed as a problem".
    episodeInterval: {
      hours: 24,
      because:
