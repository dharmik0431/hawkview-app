...
export type SendState =
  | 'READY' | 'CLAIMED' | 'SENT' | 'EXHAUSTED' | 'GAVE_UP' | 'CANCELLED'
  /** NOBODY PRESSED ANYTHING, AND THAT IS THE WHOLE DISTINCTION. The alert type was switched off,
   * the recipient went away, or the incident stopped being actionable between queueing and
   * sending — so the send stopped being appropriate and the job leaves the queue saying which.
   * The reason is closed (`WithdrawnReason` in `send-worker.ts`) and there is deliberately no
   * `withdrawn_by`: see the note below. */
  | 'WITHDRAWN'
...
/** `WITHDRAWN` IS THE THIRD SILENCE, AND THE DATABASE IS WHAT ESTABLISHED THAT IT NEEDED TO BE.
 *
 * The worker had a `withdraw` seam with no honest state to write. Every candidate was a lie:
 * CANCELLED sends a reader looking for who stopped it and they find nobody; GAVE_UP is the
 * address refusing us, the same silence with the opposite meaning.
 *
 * **AND THE SHORTCUT WAS NOT AVAILABLE ANYWAY — `alert_send_jobs_cancellation_check` IS A
 * BICONDITIONAL.** state = 'CANCELLED' iff `cancelled_at`, `cancelled_by` and `cancelled_because`
 * are all non-null, so writing a withdrawal as CANCELLED requires inventing a `cancelled_by`.
 * The constraint refused a lie — a database enforcing a product distinction the code was about to
 * blur — and it is why this state exists rather than a comment apologising for a reused one.
 *
 * SO THERE IS NO `withdrawn_by`, deliberately and not as an omission. There is no person to name.
 * A nullable one would be filled in eventually by somebody being helpful, and a system decision
 * would acquire an author. */

/** The states from which no further attempt may be made. Exported so a caller cannot maintain a
 * second, drifting copy of the list — a rule that was being broken by `claimStatement` below at
 * the moment `WITHDRAWN` was added to it. */
export const TERMINAL: readonly SendState[] =
  ['SENT', 'EXHAUSTED', 'GAVE_UP', 'CANCELLED', 'WITHDRAWN']

/** The same list as a SQL literal, built from `TERMINAL` rather than typed out beside it.
 *
 * **THIS EXISTS BECAUSE THE DRIFT HAD ALREADY HAPPENED.** `claimStatement`'s WHERE clause carried
 * its own hand-written copy of the terminal states, eight lines under a comment promising there
 * was only one list. Adding `WITHDRAWN` to `TERMINAL` alone would have left a withdrawn job
 * claimable — which is precisely the "the bound is decorative" defect that comment was written
 * about, reintroduced by the edit that quoted it.
 *
 * Safe to interpolate: every element is a compile-time literal of a closed union, so there is no
 * value here that any input reaches. */
...
export function claimStatement(
  ...
  return {
    sql: [
      ...
      // EXHAUSTED job could be claimed again, which makes the bound decorative. Built from
      // `TERMINAL` rather than repeated: this line held its own copy until `WITHDRAWN` was added
...
// ---------------------------------------------------------------------------------------
// WITHDRAWAL. The send stopped being appropriate, and no person decided that.
// ---------------------------------------------------------------------------------------

export interface WithdrawStatement {
  readonly sql: string
  readonly params: readonly unknown[]
  /** One if the job was still live, zero if it had already finished — and **zero is not an
   * error**. A job that reached `SENT` between the resolve and this write was sent; relabelling it
   * would be the record lying about what the product did. The worker treats zero as "somebody
   * else settled it first" and moves on, exactly as it does for a lost claim. */
  readonly expectedRowCount: 1
}

/** Settle a job as withdrawn, as ONE conditional UPDATE.
 *
 * **THE REASON IS PART OF THE WRITE, NOT A SECOND STATEMENT.** `alert_send_jobs_withdrawal_check`
 * is a biconditional — state = 'WITHDRAWN' iff `withdrawn_at` and `withdrawn_because` are both
 * non-null — so a half-written withdrawal is refused by the database rather than surviving as a
 * row that reads as live and carries a reason. That is the same shape as the cancellation check,
 * and it is the constraint that made this state necessary in the first place.
 *
 * THE GUARD IS THE TERMINAL LIST, FOR THE REASON `claimStatement` CARRIES. The resolve happens
 * after the claim, so in the ordinary case this job is CLAIMED and nothing else can settle it.
 * The guard is for the case where that is not true — a claim that expired mid-resolve, a stop
 * pressed in between — and in both the right answer is to leave the finished row alone.
 *
 * NO `withdrawn_by` COLUMN EXISTS TO SET. See the note on `SendState`: there is no person, and a
 * nullable column for one would be filled in eventually by somebody being helpful. */
export function withdrawStatement(
  ...
  return {
    sql: [
      'UPDATE alert_send_jobs',
      "SET state = 'WITHDRAWN', withdrawn_at = $3::timestamptz, withdrawn_because = $2,",
      // Released in the same write. A withdrawn job holding a claim would keep a dead worker's
      // name on a row nobody will ever claim again, and `claimed_at` is what "in flight" is read
      // from — see the claim-provenance CHECK, which takes all three or none.
      '    claimed_by = NULL, claimed_at = NULL, claim_expires_at = NULL',
      'WHERE message_id = $1',
      `  AND state NOT IN (${TERMINAL_SQL})`,
    ].join('\n'),
    params: [messageId, because, atIso],
    expectedRowCount: 1,
  }
}
