...
test('TWO DIFFERENT UNREADABLE OPERATORS ARE NOT THE SAME OPERATOR', () => {
  // The reflexive half of the operator split, which did not come with the rest of it.
  // Written to FAIL against the code as it stands, because 35 passing tests over a
  // behaviour nothing covers is not validation — it is the absence of a test.
  //
  // The mechanism: `operatorChanged` compares the MAPPED operator, and two different
  // unreadable operators both map to UNRECOGNISED. So the "nothing changed" guard returns
  // before reaching the grant_operator_unknown branch written for exactly this input. The
  // branch is correct and unreachable.
  const verdict = classifyConditionalAccessChange(
    policy({ grantOperator: 'UNRECOGNISED', grantControls: ['mfa', 'compliantDevice'] }),
    policy({ grantOperator: 'UNRECOGNISED', grantControls: ['mfa', 'compliantDevice'] }))
  assert.equal(verdict.classification, 'UNCLASSIFIED',
    'an operator we cannot read is not evidence that it did not change')
  assert.equal(verdict.rule, 'conditional_access.grant_operator_unknown')

  // THE ROOT, SWEPT RATHER THAN PATCHED: this is the second time a comparison in projection
  // space has hidden a change — the state defect was the first. The rule that separates the
  // safe cases from this one is that A DISTINGUISHED-VALUE CHECK MUST NOT SIT BEHIND A
  // CHANGE PREDICATE. The state checks and the unreadable-list check are unconditional and
  // were never affected; this one was gated behind "did anything change", computed over the
  // very projection that collapses the two values.
  //
  // The other three directions already worked and must keep working.
  for (const [before, after] of [
    ['AND', 'UNRECOGNISED'], ['UNRECOGNISED', 'AND'], ['ABSENT', 'UNRECOGNISED'],
  ] as const) {
    const other = classifyConditionalAccessChange(
      policy({ grantOperator: before, grantControls: ['mfa', 'compliantDevice'] }),
      policy({ grantOperator: after, grantControls: ['mfa', 'compliantDevice'] }))
    assert.equal(other.classification, 'UNCLASSIFIED', `${before} -> ${after}`)
  }

  // POSITIVE CONTROL: two READABLE operators that genuinely did not change still reach
  // routine, so the fix is about the unreadable value rather than about the guard being
  // removed.
  assert.equal(
    classifyConditionalAccessChange(
      policy({ grantOperator: 'AND' }), policy({ grantOperator: 'AND' })).classification,
    'ROUTINE')

  // AND THE CASE THAT STAYS ROUTINE, stated so it is a decision rather than a gap: an
  // unreadable operator on a policy with NO grant controls has nothing to combine and
  // nothing to direct. A malformed grantControls object would surface as an unreadable
  // LIST, which is caught unconditionally before this point, so "no controls" here is
  // genuinely no controls rather than an unread field.
  assert.equal(
    classifyConditionalAccessChange(
      policy({ grantOperator: 'UNRECOGNISED', grantControls: [] }),
      policy({ grantOperator: 'UNRECOGNISED', grantControls: [] })).classification,
    'ROUTINE')
})

test('AN ABSENT OPERATOR IS KNOWABLY UNCHANGED, which is why it is not treated as unreadable', () => {
  // The other half of the reflexive fix, and a mutation survived until this existed:
  // replacing the UNRECOGNISED test with one that also catches ABSENT passed everything.
  //
  // The distinction is the whole reason the two were split. Absent is absent — there is no
  // hidden variation behind it, so "nothing changed" is knowable and TRUE. UNRECOGNISED is
  // the value that cannot support that claim. Treating them alike would report impact-unknown
  // on every comparison of an unchanged operator-less policy, which is noise generated by a
  // safety rule, and that is how safety rules get removed.
  const unchanged = classifyConditionalAccessChange(
    policy({ grantOperator: 'ABSENT', grantControls: ['mfa', 'compliantDevice'] }),
    policy({ grantOperator: 'ABSENT', grantControls: ['mfa', 'compliantDevice'] }))
  assert.equal(unchanged.classification, 'ROUTINE',
    'an absent operator that did not change is not a finding')

  // POSITIVE CONTROL: the same absent operator WITH a control change does report, because
  // then there is a change we cannot direct. The exemption is about the absence of a change,
  // not about the value.
  const changed = classifyConditionalAccessChange(
    policy({ grantOperator: 'ABSENT', grantControls: ['mfa', 'compliantDevice'] }),
    policy({ grantOperator: 'ABSENT', grantControls: ['mfa'] }))
  assert.equal(changed.classification, 'UNCLASSIFIED')
  assert.equal(changed.rule, 'conditional_access.grant_operator_absent')

  // And the contrast that makes the split earn its place: the SAME shape with an unreadable
  // operator and no change at all does report, because there the sameness is unknowable.
  const unreadable = classifyConditionalAccessChange(
    policy({ grantOperator: 'UNRECOGNISED', grantControls: ['mfa', 'compliantDevice'] }),
    policy({ grantOperator: 'UNRECOGNISED', grantControls: ['mfa', 'compliantDevice'] }))
  assert.equal(unreadable.classification, 'UNCLASSIFIED')
