...
export function deriveTenantHealth(input: HealthInput) {
  ...
  const failedJobs = input.syncStates.filter((s) => s.status === 'FAILED').length; const pendingJobs = input.syncStates.filter((s) => s.status === 'RUNNING').length; const partialJobs = input.syncStates.filter((s) => s.status === 'FAILED' && s.lastSuccessfulAt !== null).length; const operations = { status: failedJobs >= 3 ? 'FAILED' as OperationsStatus : failedJobs > 0 ? 'DEGRADED' as OperationsStatus : pendingJobs > 0 ? 'SYNCING' as OperationsStatus : input.syncStates.length ? 'HEALTHY' as OperationsStatus : 'UNKNOWN' as OperationsStatus, activeIssues: failedJobs + requiredProblems.length, failedJobs, partialJobs, pendingJobs, lastSuccessfulJobAt: latestDate(input.syncStates.map((s) => s.lastSuccessfulAt)), issues: input.syncStates.filter((s) => s.status === 'FAILED').map((s) => ({ resourceType: s.resourceType, reasonCode: s.lastErrorCode, message: s.lastErrorMessage, consecutiveFailures: s.consecutiveFailures })) }
