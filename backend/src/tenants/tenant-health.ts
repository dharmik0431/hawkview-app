...
import type { MicrosoftRiskSummary } from '../identity-risk/microsoft-risk-summary.js'
...
export type ResourceClassification = 'SUCCESS' | 'EMPTY' | 'PARTIAL' | 'PENDING' | 'FAILED' | 'STALE' | 'UNSUPPORTED' | 'NOT_LICENSED' | 'PERMISSION_REQUIRED' | 'NOT_CONFIGURED'
...
function retryableSyncState(state: SyncState | undefined) { return Boolean(state && ['MICROSOFT_TRANSIENT', 'MICROSOFT_THROTTLED', 'MICROSOFT_NETWORK_TIMEOUT', 'MICROSOFT_DELTA_RESET_REQUIRED'].includes(state.lastErrorCode ?? '')) }
function isSignInFallback(state: SyncState | undefined) { return Boolean(state?.resourceType === 'SIGN_INS' && /sign-ins-.*fallback-active/.test(state.lastErrorCode ?? '')) }
function currentLimitedFallback(state: SyncState | undefined, now: Date) {
  return Boolean(
    state?.status === 'RUNNING' &&
    state.lastSuccessfulAt &&
    isSignInFallback(state) &&
    now.getTime() - state.lastSuccessfulAt.getTime() <= freshnessWindow('SIGN_INS').agingMs,
  )
}
function syncAction(tenantId: string, resourceType: string, state?: SyncState) { const id = encodeURIComponent(tenantId); if (retryableSyncState(state)) return { label: 'View synchronization', url: `/tenants/${id}/settings?section=sync&resource=${encodeURIComponent(resourceType)}` }; const actions: Record<string, { label: string; url: string }> = { AUTH_REGISTRATIONS: { label: 'Review authentication', url: securityUrl(tenantId, 'auth') }, CONDITIONAL_ACCESS: { label: 'Review policies', url: securityUrl(tenantId, 'policies') }, SIGN_INS: { label: 'Review sign-ins', url: securityUrl(tenantId, 'sign-ins') }, GROUPS: { label: 'Review groups', url: `/tenants/${id}?entraTab=groups` }, M365_AUDIT: { label: 'Review audit synchronization', url: `/tenants/${id}/settings?section=sync&resource=M365_AUDIT` } }; return actions[resourceType] ?? { label: 'Investigate', url: `/tenants/${id}/settings` } }
...
function classify(state: SyncState | undefined, now: Date): Omit<ResourceHealth, 'resourceType' | 'required'> {
  ...
  const authFailure = state.lastErrorCode === '401' || state.lastErrorCode === '403' || state.lastErrorCode === 'MICROSOFT_PERMISSION_REQUIRED' || /unauthorized|forbidden|permission|consent/i.test(state.lastErrorMessage ?? '')
  if (state.status === 'FAILED' && retryableSyncState(state) && state.consecutiveFailures < 3) return { ...base, classification: 'PENDING', reasonCode: state.lastErrorCode, message: sanitizeHealthMessage(state.lastErrorMessage) }
  ...
  if (currentLimitedFallback(state, now)) return { ...base, classification: 'PARTIAL', reasonCode: state.lastErrorCode, message: sanitizeHealthMessage(state.lastErrorMessage) ?? 'Current limited sign-in evidence is available from the Microsoft 365 audit feed.' }
  if (state.status === 'RUNNING' && isSignInFallback(state) && state.lastSuccessfulAt) return { ...base, classification: 'STALE', reasonCode: 'sign-in-fallback-stale', message: 'Limited sign-in evidence from the Microsoft 365 audit feed is no longer current.' }
...
function syncIssueLabel(resourceType: string) {
...
export function deriveTenantHealth(input: HealthInput & { microsoftRiskSummary?: MicrosoftRiskSummary }) {
  ...
  const states = new Map(input.syncStates.map((state) => [state.resourceType, state])); const notApplicable = new Set(input.notApplicableResourceTypes ?? []); const unsupportedTypes = new Set(input.unsupportedResourceTypes ?? []); const resources: ResourceHealth[] = TENANT_HEALTH_RESOURCE_REGISTRY.map((item) => unsupportedTypes.has(item.resourceType)
  ...
  // remain actionable.  This keeps the directory's issue list aligned with
  // the tenant action center instead of silently dropping failures such as
  // SIGN_INS.
  const attentionProblems = resources.filter((resource) =>
  ...
  const data = { status: dataStatus, freshnessStatus: currentFreshness, completenessPercent, successfulResources: successful.length, expectedResources: applicable.length, pendingResources: pending.length, failedResources: failed.length, unsupportedResources: unsupported.length, notLicensedResources: notLicensed.length, staleResources: stale.length, lastSyncStartedAt: latestDate(input.syncStates.map((s) => s.lastAttemptAt)), lastSyncCompletedAt: latestDate(input.syncStates.map((s) => s.lastSuccessfulAt)), issues: resources.filter((r) => r.classification !== 'SUCCESS').map((r) => ({ resourceType: r.resourceType, classification: r.classification, reasonCode: r.reasonCode, message: r.message })) }

  const microsoftRiskSummary = input.microsoftRiskSummary
  const observedRiskCount = microsoftRiskSummary?.observedActiveDistinctUserCount ?? null
  if (microsoftRiskSummary && microsoftRiskSummary.availability !== 'UNAVAILABLE' && observedRiskCount !== null && observedRiskCount > 0) {
    const exact = microsoftRiskSummary.availability === 'AVAILABLE'
    attention.push({
      key: 'risky-identities',
      label: exact ? `${observedRiskCount} active Microsoft risk ${observedRiskCount === 1 ? 'identity' : 'identities'}` : `${observedRiskCount} ${observedRiskCount === 1 ? 'identity has' : 'identities have'} active Microsoft-risk evidence requiring review`,
      severity: observedRiskCount >= 5 ? 'critical' : 'high',
      why: exact ? 'Current Microsoft Identity Protection evidence reports active risk. HawkView findings are a separate evidence channel.' : 'Microsoft Identity Protection evidence is incomplete or conflicting. Active observations require review; the exact current active total is unknown.',
      detectedAt: microsoftRiskSummary.snapshotObservedAt,
      actionLabel: 'Review Microsoft risk',
      actionUrl: `/tenants/${encodeURIComponent(input.tenantId)}/risky-users`,
    })
  }
  const coverage = mfaCoverage(input.authSnapshot?.payload); if (coverage !== null && coverage < 85) attention.push({ key: 'mfa-coverage', label: `This organization has ${coverage}% MFA registration coverage`, severity: coverage < 50 ? 'high' : 'medium', why: `HawkView recommends at least 85% registration coverage. ${100 - coverage}% of synchronized users do not have an MFA method registered. This does not establish whether MFA is required by Conditional Access, security defaults, or per-user MFA.`, detectedAt: input.authSnapshot?.observedAt.toISOString() ?? null, actionLabel: 'Review MFA registration', actionUrl: securityUrl(input.tenantId, 'auth') })
  const limitedSignIns = resources.find((resource) => resource.resourceType === 'SIGN_INS' && resource.classification === 'PARTIAL')
  if (limitedSignIns) attention.push({ key: 'sign-ins-limited-evidence', label: 'Limited sign-in evidence is active', severity: 'low', why: limitedSignIns.message ?? 'HawkView has current audit-feed login evidence; full Microsoft Graph sign-in details are unavailable.', detectedAt: limitedSignIns.lastSuccessfulAt, actionLabel: 'Review sign-in coverage', actionUrl: syncAction(input.tenantId, 'SIGN_INS').url })
