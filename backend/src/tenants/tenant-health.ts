...
export function deriveTenantHealth(input: HealthInput) {
  ...
  for (const resource of attentionProblems) { const state = states.get(resource.resourceType); const action = syncAction(input.tenantId, resource.resourceType); attention.push({ key: `sync-${resource.resourceType.toLowerCase()}`, label: syncIssueLabel(resource.resourceType), severity: state?.consecutiveFailures && state.consecutiveFailures >= 3 ? 'critical' : 'high', why: resource.message ?? 'Collection is not current.', detectedAt: resource.lastAttemptAt, actionLabel: action.label, actionUrl: action.url }) }
