import {
  ...
  type Detector, type EvidenceState, type Finding, type WithheldReason, type ZeroClaim,
} from './contract.js'
...
export function zeroClaim(state: EvidenceState, coverage: Coverage, withinBudget: boolean): ZeroClaim {
...
export function evaluate<Event>(input: Readonly<{
  ...
  // A detector that cannot run is that detector's failure. It must not erase
  // findings its neighbours already produced from the same evidence.
  for (const detector of input.detectors) findings.push(...detector.findings(applicable))

  const state = evidenceState(counted, input.collected, input.readable)
  const claim = zeroClaim(state, counted, withinBudget)
  return {
    state,
    coverage: counted,
    findings,
