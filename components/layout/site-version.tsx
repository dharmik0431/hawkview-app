...
export function SiteVersion({ align = 'start', placement = 'bottom' }: { align?: 'start' | 'center'; placement?: 'top' | 'bottom' }) {
  ...
  if (identity.kind !== 'build' && identity.kind !== 'source') {
    return (
  ...
  return (
    <details className="relative inline-block max-w-full text-left text-[11px] leading-5 text-slate-600 dark:text-slate-400">
      <summary className="cursor-pointer whitespace-nowrap rounded-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500" aria-label={`Frontend site version ${identity.sourceHash.slice(0, 12)}; show version details`}>
      ...
      <div className={`absolute z-50 w-64 rounded-lg border border-slate-200 bg-white p-3 text-xs leading-5 text-slate-700 shadow-lg dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200 ${placement === 'top' ? 'bottom-full mb-2' : 'top-full mt-2'} ${align === 'center' ? 'left-1/2 max-w-[calc(100vw-2rem)] -translate-x-1/2' : 'left-0 max-w-[calc(100vw-7rem)]'}`}>
        <p className="font-semibold">Frontend version details</p>
        ...
        <p className="mt-2 text-slate-500 dark:text-slate-400">{identity.kind === 'source' ? 'Identifies the frontend source used for this compilation; runtime settings are separate.' : 'Identifies this frontend’s source build; runtime settings are separate.'}</p>
      </div>
    </details>
  )
}
