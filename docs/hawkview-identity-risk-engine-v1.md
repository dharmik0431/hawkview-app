...
The initial implementation retains these bounded operational control events for 90 days, matching the maximum evaluation-run retention. The existing scheduler-authenticated internal maintenance path removes at most 500 expired events per run and records one tenant-data-free global maintenance event per five-minute retry bucket. Repeated and concurrent runs are idempotent; a backlog drains over later scheduler runs. Exact-scope pruning remains available for incident cleanup and always retains both the scope type and organization-plus-customer-tenant opaque-scope predicate. This duration remains subject to PM, security, privacy, and legal review; operational events must never be retained indefinitely by default.
...
Ordinary `NOT_MATCHED` and `NOT_EVALUATED` outcomes live only in memory long enough to increment run/rule counters. They must not create user × rule × run rows. Derived assessment graphs have 90 x 24 hours of physical retention from their original authoritative run creation time, including no-source/UNAVAILABLE checks. Current-read validity remains `min(trusted platform evaluation time + 90 days, earliest contributing source expiry)`; new no-source checks have a 90-day ceiling but remain unavailable. Legacy seven-day validity is not extended or used to authorize early physical deletion. See [physical retention versus validity](identity-risk-history-retention.md). Caller-supplied evaluation or evidence timestamps never establish retention or lease authority. Diagnostic samples, if implemented, keep their separate short-lived specification; this slice does not add them. Partial or failed runs are never presented as complete coverage.
