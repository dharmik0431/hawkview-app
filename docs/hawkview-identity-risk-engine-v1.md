...
Ordinary `NOT_MATCHED` and `NOT_EVALUATED` outcomes live only in memory long enough to increment run/rule counters. They must not create user × rule × run rows. Completed run and rule-coverage aggregates expire at `min(trusted platform evaluation time + 90 days, earliest contributing source expiry)`. A run/rule aggregate with no contributing source expires at `trusted platform evaluation time + 7 days`. Caller-supplied evaluation or evidence timestamps never establish retention or lease authority. Diagnostic samples expire at `min(createdAt + 7 days, earliest contributing source expiry)`; a no-source abstention sample expires after 7 days. Partial or failed runs are marked failed and are never presented as complete coverage.
