...
test('saving says when the change takes effect', () => {
  ...
  // AND THAT ALERTS ALREADY RAISED KEEP THEIR URGENCY. The pipeline records one decision per
  // finding and never revisits it, so a tier change no longer restyles what is already on the
  // bell. It used to, through an ON CONFLICT DO UPDATE that also un-dismissed things; the
  // sentence has to follow the behaviour rather than the other way round.
...
test('an unmapped type does not claim its alerts are delivered', () => {
  ...
  // sentence is already the quiet one, which is exactly why a contradiction
  // there would be easy to leave in. That sentence used to read "Recorded and
  // visible here. Not delivered." and became false when the pipeline stopped
  // writing the notification row for a silenced type.
  for (const tier of TIERS) {
...
test('record only does not promise the alert is visible here', () => {
  // THE COPY WAS FALSE FOR AS LONG AS IT TOOK TO NOTICE. It read "Recorded and
  // visible here. Not delivered." while the pipeline had stopped writing the
  // notification row -- so the page told the person choosing that they would
  // still see the alert in HawkView, and they would not. Nothing failed when
  // the behaviour moved, because no test pinned the sentence.
  //
  // ASSERTED AS A PROPERTY, NOT A STRING. Pinning the exact wording would fail
  // on any rewrite and prove nothing about meaning; what must stay true is that
  // the tier declares no live channel and the sentence does not promise one.
  assert.deepEqual(
    TIER_CHANNELS.RECORD_ONLY.filter((entry) => entry.live),
    [],
    'record only declared a live channel, so the page will promise delivery'
  )

  const mapped = deliveryDescription('RECORD_ONLY', true)
  assert.doesNotMatch(
    mapped.today,
    /visible here|visible in|delivered by/i,
    'record only promised visibility or delivery: ' + mapped.today
  )
  assert.match(mapped.today, /[Rr]ecorded/, 'it must still say the evidence is kept')

  // AND IT MUST SAY WHERE. "Evidence you can look up" was the first wording and it pointed
  // nowhere: there is no reader for the alert record at all, and the thing an MSP can actually
  // open is the finding on the risky user. A promise without a destination is the kind of
  // sentence somebody goes looking for and does not find.
  assert.match(mapped.today, /risky user/i, 'it must name where the evidence can be found')

  // THE CONTROL. A tier that IS delivered must still say so, or the assertion
  // above would be satisfied by a page that promises nothing to anybody.
  assert.match(deliveryDescription('ACT_TODAY', true).today, /Delivered by/)
})
