...
/**
 * Built for the coherence pass rather than for a single state: the same
 * identities deliberately appear in several sections at once — Microsoft's risk
 * group, Microsoft's cleared group, the counted list and the supporting
 * evidence — so contradictions between sections have somewhere to surface.
 */
function crossSection() {
  const value = correlatedFindings()
  const mailbox = assessmentUser('HV-ID-MBX-001.v1', 'd')
  mailbox.label = 'Alice Chen'
  value.users.push(mailbox)
  Object.assign(value.rules[2], { assessedIdentities: 3, matchedIdentities: 2 })
  return value
}
...
const routes = {
  ...
  '/coherence': {
    title:
      'Coherence pass — the same people in several sections, and seen versus heard',
    fixture: crossSection,
    microsoft: microsoftCorrelated,
    coherence: [
      'Alice Chen',
