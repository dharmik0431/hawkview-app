...
Status: merged in PR #154. A P0-6.1 local reliability follow-up is investigating live SharePoint administrative-enrichment failures and the Exchange Admin collector's scheduling gap; it is not published or deployed.
...
- Deployment prerequisite: apply `20260818195000_add_tenant_license_service_plans` before or with the matching backend deployment. This change is local only: nothing has been committed, pushed, deployed, migrated, or applied to live tenants. GAS remains independently user-managed and has not been pulled or published by this worktree.
- Render live logs/state were not accessible from this local worktree, so the specific current SharePoint upstream HTTP/tenant/RBAC result remains unverified. The code does not infer missing permission merely from the generic persisted refusal.
