...
Status: implemented and repaired locally on `codex/p0-8-exchange-rule-investigation`, based on authenticated `origin/main` merge `45a06db`. No commit, PR, merge, deployment, live synchronization, permission, consent, schema, migration, or tenant change has occurred. Independent QA first failed with **P0=0, P1=4, P2=3**; a narrow follow-up QA then found the rule-focus refs attached to the mailbox drawer. Both repair rounds are complete locally and awaiting final independent re-QA. This section does not claim release approval.
